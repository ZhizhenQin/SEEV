...
class ObsAvoidSI(ControlAffineSystem):
    ...
    def __init__(
        ...
        self.unsafe_range = 0.2 
        self.safe_range = 0.2 + buffer_range
    ...
    def n_controls(self) -> int:
        return ObsAvoidSI.N_CONTROLS
    
    @property
    def umax(self):
        return ObsAvoidSI.MAX_VELOCITY
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        upper_limit = torch.tensor([ObsAvoidSI.MAX_POSITION, ObsAvoidSI.MAX_POSITION])
        lower_limit = torch.tensor([-ObsAvoidSI.MAX_POSITION, -ObsAvoidSI.MAX_POSITION])
    ...
    def control_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        upper_limit = torch.tensor([ObsAvoidSI.MAX_VELOCITY, ObsAvoidSI.MAX_VELOCITY])
        lower_limit = torch.tensor([-ObsAvoidSI.MAX_VELOCITY, -ObsAvoidSI.MAX_VELOCITY])
        return (upper_limit, lower_limit)
    ...
    def safe_term(self, x):
        ...
        distance = x[:, : ObsAvoidSI.Y + 1].norm(dim=-1, p=order)
        return distance - (ObsAvoidSI.SAFE_RANGE + self.buffer_range)
    ...
    def safe_mask(self, x):
        ...
        order = 1 if hasattr(self, "use_l1_norm") and self.use_l1_norm else 2
        distance = x[:, : ObsAvoidSI.Y + 1].norm(dim=-1, p=order)
        safe_mask = distance > (ObsAvoidSI.SAFE_RANGE + self.buffer_range)
        return safe_mask
    
#     @property
#     def beta(self):
#         # minimal unsafe_term
#         return -ObsAvoidSI.UNSAFE_RANGE
    ...
    def unsafe_term(self, x):
        ...
        order = 1 if hasattr(self, "use_l1_norm") and self.use_l1_norm else 2
        distance = x[:, : ObsAvoidSI.Y + 1].norm(dim=-1, p=order)
        return distance - ObsAvoidSI.UNSAFE_RANGE
...
#         ]).T.unsqueeze(dim = 1)
    ...
    def unsafe_mask(self, x):
        ...
        order = 1 if hasattr(self, "use_l1_norm") and self.use_l1_norm else 2
        distance = x[:, : ObsAvoidSI.Y + 1].norm(dim=-1, p=order)
        unsafe_mask = distance <= ObsAvoidSI.UNSAFE_RANGE
        return unsafe_mask
    ...
    def _g(self, x: torch.Tensor, params: Scenario):
        ...
        g[:, ObsAvoidSI.X, ObsAvoidSI.U0] = 1.0
        g[:, ObsAvoidSI.Y, ObsAvoidSI.U1] = 1.0
        return g
